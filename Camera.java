...
public class Camera 
{
	...
	Camera(float inner_ringdius, float outer_ringdius, Planet planet, Box2DProcessing box, PApplet par)
	{
		this.box2d = box;
		parent = par;
	  ...
	  void display()
	  {
		  ...
		  parent.fill(100);
		  parent.ellipse(pos.x, pos.y, 10,10);
	  }
	  
	  private float rdbt(float min, float max)
		{
			return (float)parent.random(1)*(max-min)+min;
		}
}
