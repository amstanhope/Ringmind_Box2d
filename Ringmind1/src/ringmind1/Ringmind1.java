...
public class Ringmind1 extends PApplet 
{
	...
	//colors
	int backColor = color(27, 38, 44);
	int gravColor = color(0,183,194);
	int planetColor = color(5, 219, 242);
	int rockColor = color(0);
	...
	private void init()
	{
		...
		for(int i=0;i<partis;i++)
		{
			...
			Particle p = new Particle(x, y, rockDensity, iceDensity,box2d);
			p.setOrbitVelocity(planet, G, planetMass);
			pars.add(p);
		}

		camera = new Camera(inner_ringdius, outter_ringdius, planet,box2d);
 ...
 /* 
	...
	{
		new Merge(cp, box2d, muns,pars,toaddnremove, relativeSize, relativeSizeRange, combinedSize, combinedSizeRange,rockDensity,iceDensity);
	}
