...
public class Merge extends PApplet
{
	...
	Merge(Contact cp, Box2DProcessing box, ArrayList<Mun> muns, ArrayList<Particle> pars, ArrayList<Merge> toaddnremove, float relativeSize, float relativeSizeRange, float combinedSize, float combinedSizeRange, float rockDensity, float iceDensity)
	{
		this.box2d = box;
		burst = false;
  ...
  //
	void reborn(float moon_min, ArrayList<Mun> muns, ArrayList<Particle> pars)
	{
		if(this.burst)
		{
			...
			Particle newParti = new Particle(this.pos.x, this.pos.y, m, rockPercent,box2d);
			pars.add(newParti);
	...
	void make_burst(Vec2 pos, float mass, Vec2 vel, int burstNum, float burstAccel, ArrayList<Particle> pars)
	{
		...
		for(int i=burstNum-1;i>=0;i--)
		{
			...
			Particle newp = new Particle(x,y,newm, rockPercent,box2d);
